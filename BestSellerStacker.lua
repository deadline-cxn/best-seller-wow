  ...
  for bag = 0,4
	...
	for slot = 1,20
	  ...
	  if(iHi==nil) then iHi="wot"
		...
		if(link ~=nil) then
		  itemName,itemLink,itemRarity,itemLevelReq,itemType,itemSubType,itemStackCount = GetItemInfo(link)
		  duh,count,d2,quality,uread = GetContainerItemInfo(bag,slot)
			  ...
			  for islot = 1,30
				...
				if(ibag==bag) and (islot==slot) then
				  ...
				  if(GetContainerItemLink(ibag,islot)==iHi) then
					link=Jelly_GetLink(iHi)
					if(link ~= nil) then
					  itemName,itemLink,itemRarity,itemLevelReq,itemMinLevel,itemType,itemSubType,itemStackCount,itemEquipLoc,itemTexture = GetItemInfo(link)
					  duh,count,d2,quality,uread = GetContainerItemInfo(ibag,islot)
					  if(count==itemStackCount) then
					  else
