...
function bsAutoSell()
    ...
    if(db.AutoSell["Active"]~=1) then return; end
-- locals
    local itemName,
          itemLink,
          itemRarity,
...
function bsWasAutoSold(item)
    for x,y in pairs(bsRecentAutoSold) do
        --bsDPrint(" item:"..item.." x:"..x.." y:"..y);
        if(x==item) then
            if(y==1) then
                --bsDPrint("Does match returning 1");
                return 1;
            end
        end
    end
...
function bsIsQuestItem(bsitem)
    ...
    bsDPrint("isQuestItem: numEntries:"..numEntries.." numQuests:"..numQuests);
    for bsqu=1,numQuests do
        bsnlb=GetNumQuestLeaderBoards(bsqu);
        for bslb=1,bsnlb do
            local bstxt,bstyp,bsfin = GetQuestLogLeaderBoard(bslb,bsqu);
            if(bstyp=="item") then
                if(bstxt==nil) then txt="(bstxt nil)"; end
                if(bstyp==nil) then type="(bstyp nil)"; end
                if(bsfin==nil) then fin="(bsfin nil)"; end
                for bsit,_ in string.gmatch(bstxt,"(.+):(.+)") do
                    bsDPrint("QUEST ITEM >> "..bsitem.." <<"..bsit.." "..bstyp);
                    if(bsitem==bsit) then
                        bsDPrint("QUEST ITEM >> "..bsitem.." <<"..bsit.." << A MATCH!");
                        return 1;
                    end
                end
            end
...
function bsRemoveSellItem(itemName)
    if(db.AutoSell["Items"][itemName]~=nil) then
        bsInform("Removed ["..itemName.."] from sell list");
    ...
    if(inlink~=nil) then
        ...
        if(itemRarity>0) then
            if(db.AutoSell["Items"][itemName]==1) then
                ...
                bsInform("Added ["..itemName.."] to sell list");
...
function bsRemoveExcludeItem(itemName)
    if(db.AutoSell["Exclude"][itemName]~=nil) then
        bsInform("Removed ["..itemName.."] from sell EXCLUDE list");
...
function bsAddExcludeItem(inlink)
    if(inlink~=nil) then
        local itemName, itemLink, itemRarity, itemLevelReq, itemType, itemSubType, itemStackCount = GetItemInfo(inlink);
        bsDPrint("ITEM EXCLUDE: "..itemName);
        if(db.AutoSell["Exclude"][itemName]==1) then
        else
            bsInform("Added ["..itemName.."] to EXCLUDE list");
...
function bsASLCreateButtons()
    ...
    local button      = createFrame("Button", "BSASLSF_ItemButton1",
                                     parent,  "BSASLSFButtonTemplate");
    button:SetID(1);
...
function bsASLPopulate()
    ...
    for index = 1,23 do getglobal("BSASLSF_ItemButton"..index):Hide(); end
	...
	bsDPrint("BSASLSF.RealResultsSize ["..BSASLSF.RealResultsSize.."]");
	local offset = BSASLSF.offset;
	bsDPrint("BSASLSF.offset="..offset);

    local index=1;
    local grof=0;

    -- table.sort(db.AutoSell["Items"]);
    --[[
for i = 1,n,1 do -- for each item of the table
	index_min = i;
	for j = i+1,n,1 do -- for each item of the table, is it the smallest one ?
		if (t[j] < t[index_min]) then index_min = j; end; -- yes : then keep the position
	end;
	t[i], t[index_min] = t[index_min], t[i]; -- swap current item with the smallest one we found
end
]]--
	for bsaslitem,astable in pairs (db.AutoSell["Items"]) do
		if(grof<offset) then
            ...
            bsDPrint("BSASLSF_ItemButton"..index.." RESULT :"..bsaslitem);
...
function bsASLEditBoxAdd()
    if(BSASLFrameAddItemBox:GetText()~="") then
        ...
        bsInform("Added "..BSASLFrameAddItemBox:GetText().." to auto sell list");
        bsASLPopulate();
...
function bsEXLPopulate()
	...
	bsDPrint("BSEXLSF.RealResultsSize ["..BSEXLSF.RealResultsSize.."]");
	local offset = BSEXLSF.offset;
	bsDPrint("BSEXLSF.offset="..offset);
    local index=1;
    local grof=0;
	...
	for bsaslitem,astable in pairs (db.AutoSell["Exclude"]) do
		if(grof<offset) then
            ...
            bsDPrint("BSEXLSF_ItemButton"..index.." RESULT :"..bsaslitem);
