...
function bsOnEvent(self,event,...)
  ...
  if(event=="QUEST_DETAIL")then if(db.QuestAccept==1)then AcceptQuest() end end
  if(event=="QUEST_PROGRESS")then	--[[ if(db.QuestComplete==1)then		local complete_button= _G["QuestFrameCompleteQuestButton"]			if(complete_button)then				complete_button:Click()            --CompleteQuest()            end        end	]]--
  end
