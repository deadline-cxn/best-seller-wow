function bsAutoBuy()
  ...
  sml_dprint(db.Debug,"BestSeller","Merchant has "..numItems.." items")
  ...
  for i = 1, numItems do
	...
	sml_dprint(db.Debug,"BestSeller","Merchant slot ["..i.."] ".."name:"..name..",".."price:"..price..",".."qty:"..qty..",".."num:"..num..",".."usable:"..usable..",".."extcost:"..extcost )
	...
	sml_dprint(db.Debug,"BestSeller",itemName.." stacks["..itemStackCount.."]")
	for item,buyamount in pairs (db.AutoBuy["Items"]) do
	  ...
	  if (name == item) or wild==1 then
		sml_dprint(db.Debug,"BestSeller"," WE HAVE MATCH!   >"..item.."> BUY! : WILDCARD: "..wildcard)
		local ba=buyamount
		...
		local ab=0
		sml_dprint(db.Debug,"BestSeller","buyamount["..ba.."] ".."itemstacksize["..iz.."] ".."merchantstackamt["..ms.."] ".."merchantquantity["..mq.."] ".."amountowned["..ia.."] ".."amounttobuy["..ab.."] ")
		ab=ba-ia
		sml_dprint(db.Debug,"BestSeller"," AMOUNT TO BUY: "..ab)
		if( ab > 0) then
		  sml_print("BestSeller",OFCC.." 'Best Buy' purchasing "..WFCC..name..OFCC.." x"..WFCC..ab..OFCC.." because it is in your Autobuy list")
		  if(ab>iz) then
			local wtfz=ab
			while wtfz > iz do
			  sml_dprint(db.Debug,"BestSeller"," A: Buying: "..iz)
			  BuyMerchantItem(i,iz)
			  wtfz=wtfz-iz
			end
			sml_dprint(db.Debug,"BestSeller"," B: Buying: "..wtfz)
			BuyMerchantItem(i,wtfz)
		  else
			sml_dprint(db.Debug,"BestSeller"," C: Buying: "..ab)
...
function bsABList()
  ...
  for item,buyamount in pairs (db.AutoBuy["Items"]) do
	sml_print("BestSeller","Auto Buy: "..item.. " ("..buyamount..")")
  end
end
...
function bsABRemove(name)
  sml_dprint(db.Debug,"BestSeller","REMOVED "..name)
...
function bsABLPopulate()
  ...
  sml_dprint(db.Debug,"BestSeller","ABL Pop")
  ...
  sml_dprint(db.Debug,"BestSeller","BSABLSF.RealResultsSize ["..BSABLSF.RealResultsSize.."]")
  local offset = BSABLSF.offset
  sml_dprint(db.Debug,"BestSeller","BSABLSF.offset="..offset)
  local index=1
  local grof=0
  if(db.AutoBuy["Items"]~=nil) then
	for bsaslitem,astable in pairs (db.AutoBuy["Items"]) do
	  if(grof<offset) then
		...
		if(zbutton~=nil) then
		  ...
		  sml_dprint(db.Debug,"BestSeller","ERROR: zbutton==nil BestSelleraAutoBuy.lua Line 150")
		end
...
function bsABLButtonPressed(index)
  sml_dprint(db.Debug,"BestSeller","Autobuy List "..index)
